...
- 开启认购时，认购额度进入 `RuyiBeastSaleVault`，用户在 BSC 上支付 BNB 认购，Token 直接发到用户钱包。
...
- 持币用户可绑定本命号码；号码中奖后，可领取对应轮次的风险池奖励。
- 项目方可配置 DEX 路由、交易对、回购接收地址、LP 接收地址，并开启自动回购和自动加池。
- 自动 DEX 处理达到阈值后会在卖出交易中尝试触发：合约把金库池 Token 换成底池资产，再按配置执行回购和加池；如果 DEX 临时失败，不会卡死用户交易，税池余额保留后续再处理。
